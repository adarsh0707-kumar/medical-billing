...
| Auditability     | **Solid for writes** — actor, model and before/after recorded for every write to master data. Reads are deliberately not logged (see [03 §3.12](./03-data-model.md#312-auditlog--who-changed-what)) |
...
**Rotation.** Both secrets have a procedure — ordered steps, what breaks mid-rotation, how to confirm it worked and how to back out — in [06 § Rotating a secret](./06-development-guide.md#rotating-a-secret). The two shapes in one line each: rotating `JWT_SECRET` invalidates every session at once, which is blunt and is also the only system-wide revocation lever there is (§10 P1-7); rotating the **database** password appears to do nothing, because Postgres applies `POSTGRES_USER`/`POSTGRES_PASSWORD` only when it initialises an *empty* data directory, so editing `.env.prod` on a running system leaves the role untouched and the API then fails `P1000`. It is changed inside the database with `ALTER ROLE`, or by recreating the volume from a dump.
...
- No access log of who *viewed* a customer record — deliberately; read logging is argued against in [03 §3.12](./03-data-model.md#312-auditlog--who-changed-what).
...
| T-9  | Insider data exfiltration                  | Any role could page through every customer's history                                   | Med        | Med               | **Reduced 2026-08-24** — history is ADMIN/PHARMACIST only, and every *write* to a customer record is attributed. Reads are still not logged, deliberately ([03 §3.12](./03-data-model.md#312-auditlog--who-changed-what)) | Low–Med — a pharmacist or admin can still export at will, and nothing records a read |
...
| T-14 | Forced session loss via CSRF on refresh    | Another site drives `POST /api/auth/refresh`, the one route the browser authenticates itself with a cookie. Not theft — CORS still stops the response being read — but the rotation it causes can make the victim's own next refresh look like token reuse, which revokes every session they have | Low | Med | `SameSite=Strict` outside production. In production the cookie must be `None` for the SPA to work cross-site, so the defence is an explicit `Origin` allowlist check mounted ahead of CORS (`middlewares/csrf.middleware.js`, 2026-08-31), which refuses a foreign origin with `403` and leaves the session untouched | **Low.** Before the guard the CORS origin callback was already rejecting these with an error before the router saw them — the protection existed but was incidental, unnamed and untested, and the documented way to quiet its `500` would have removed it |
...
6. **Password policy** — ~~minimum length~~ (12 since 2026-08-22) and a blocklist, both done. **The breach check was declined**, and the reasoning belongs on the record rather than looking like an oversight. HaveIBeenPwned's k-anonymity range API never transmits the password and is the standard answer, but it would be this stack's **first outbound dependency**: egress from the backend container, a third party in the path of every password change, and either a live network call in CI or a mock that proves nothing. It also has to fail open — a pharmacy must not be unable to change a password because someone else's API is down — and a check that silently passes whenever it cannot run is weaker than its presence suggests. The local blocklist covers what actually threatens this system: an operator choosing `admin123`, `pharmacy2026`, or the credential printed in the README. Revisit if self-service signup ever lands, where the population of chosen passwords stops being a handful of known staff. ~~**A forced reset flow remains open.**~~
    ...
    **The reset flow shipped 2026-08-31 (FR-AUTH-11), and it brought the outbound dependency this entry declined — so the difference is worth stating rather than letting the two read as inconsistent.** The breach check was refused because it *sat inside a control and would have failed open*: when the lookup could not run the password still changed, and the protection quietly evaporated while every screen reported it enforced. Mail is not inside a control. It is the **delivery** of something the user asked for, so a failure weakens nothing — no token is honoured, no password changes, no session ends. The user simply does not get a link, and the next thing they do is notice. This entry also said to revisit once self-service signup landed; it did, on 2026-08-29, and it changed the stakes rather than the argument — a shopkeeper who opened their own shop has **no administrator to ask**, so an emailed reset is the only way back in. The admin-initiated reset still covers staff.

    **What happens when the mail server is unreachable**, since "reset silently did nothing" is the failure that will actually occur:

    - **Misconfigured — refused, not accepted.** While `SMTP_HOST/PORT/USER/PASS/FROM` or `APP_URL` is unset, the endpoint answers `503` and issues no token: nobody is told a link is coming that this deployment cannot send. The refusal turns on the deployment's configuration and not on the address, so it is the same answer for every caller and not an enumeration oracle. `src/index.js` refused to *start* without them until 2026-09-01, which took a production API down for two days over a feature none of billing, inventory or reporting uses — `JWT_SECRET` earns a boot guard because nothing works without it; mail does not.
    - **Unreachable at send time — the response does not change.** It cannot: the endpoint answers identically for a known and an unknown address, and a `503` for one against a `200` for the other is an oracle for which addresses have accounts. The failure is logged at `error` with the request id, and the recipient — never the link — is logged with it.
    - **The residual, stated plainly.** A reset that could not be sent looks exactly like one that was, and the log is where an operator finds out. That is the cheaper of the two costs, and it is written here rather than left for somebody to learn from a user who never got their email.
7. ~~Invalidate tokens on password change and on deactivation.~~ **Done 2026-08-22 on the API; the client only started calling it on 2026-08-25.** The gap is worth recording because it is the failure mode this whole item exists to prevent: `POST /api/auth/logout` was written, tested and documented, and the SPA's **Sign out** button never called it — `logout()` in `store/auth.store.ts` cleared two `localStorage` keys, so `tokenVersion` was never bumped and the `HttpOnly` refresh cookie was never revoked. An endpoint nothing invokes revokes nothing, and every document here described the endpoint's behaviour as though it were the product's. Guarded now by `store/__tests__/auth.store.test.ts`, which asserts the request is made and that the local sign-out survives the server refusing it. A `tokenVersion` counter on `User`, compared against a claim in the token, needing no cache store (there is none since Redis was removed, [G-03](./08-gap-analysis.md#g-03)) and riding on the user reload `protect` already performs. Bumped by `POST /api/auth/logout`, by a password change, and by deactivation. The last of those closed a gap nobody had written down: `protect`'s `isActive` check only holds *while* the flag is set, so reactivating an account handed back every token that was live when it was suspended — and deactivate-then-reactivate is precisely what an administrator does to a compromised account.
...
16. ~~Document key rotation for `JWT_SECRET` and database credentials.~~ **Done 2026-09-03** — two runbooks in [06 § Rotating a secret](./06-development-guide.md#rotating-a-secret), each as ordered steps with what breaks mid-rotation, how to confirm it worked and how to back out.

    Written as procedures rather than warnings because the two fail in opposite directions, and a warning only covers one of them. `JWT_SECRET` does exactly what it is told and signs every user out doing it; the database password *appears* to change and does not, because the Postgres image reads it only when initialising an empty data directory — the operator finds out from a `P1000` after the fact. The second needed the `ALTER ROLE` path spelled out, including the part that makes it work when the old password is lost: the image grants `trust` to the container's own socket and to `127.0.0.1`, so a connection from inside the container is never challenged — one more reason Postgres publishes no port, and the reason the runbook tells the operator **not** to verify the rotation with `psql` on loopback, where any password succeeds. Rehearsed on the development stack rather than reasoned about.
