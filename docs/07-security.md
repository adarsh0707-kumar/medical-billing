...
What still blocks internet exposure is no longer transport or secrets. It is **token revocation** (a leaked token is valid for up to seven days, and a password change does not end an attacker's session) and **auditability** (no attribution for stock or price changes). Both are in §10, and neither has a configuration answer — they need code.
...
| T-12 | Repudiation of a sale                      | No audit trail beyond`Invoice.userId`                                                | Low        | Med               | Invoice authorship; a void leaves the original intact and issues a dated credit note        | Med — stock and price edits are still unattributed (P1-11)          |
| T-13 | Session survives its own compromise        | No revocation; password change does not invalidate outstanding tokens                  | Low        | High              | None — rotating`JWT_SECRET` signs out everyone at once                                    | **Med** — the highest-value item left in §10 (P1-7, P1-8)      |
