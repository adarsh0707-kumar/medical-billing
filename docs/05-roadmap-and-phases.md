...
| Production deployability                | **Deployed** — running on Render since 2026-09-02, with multi-stage images, TLS with HSTS and a CSP, no credential literals, data ports unpublished, structured logging, a readiness probe that now names the running commit, and a rehearsed restore. What remains is operator work rather than code: a real certificate, the six mail variables, and the two retention sweeps in cron |
| Test coverage                           | **741 backend tests across 30 files (91.53% statements, measured 2026-09-02)** with 90% gates on the invoice, auth and dashboard paths and 100% on the shared trend query, plus **181 frontend unit tests across 22 files** and a 7-flow browser smoke — all three on CI, all measured the same day. `manufacturer.controller.js` (70.4%), `error.middleware.js` (71.4%) and `medicine.controller.js` (77.4%) are the thin spots; `config/mailer.js` at 48.1% is lower still and stays there, because no suite opens an SMTP socket |
| Documentation accuracy                  | This`docs/` set is the reference; the component READMEs were trimmed to point at it on 2026-08-20. Reconciled against the code on 2026-09-02, which found eight stale figures — this row's own snapshot among them |

The honest read as of **2026-09-02**: the correctness gaps that made v1.0.0 unsafe for real money are closed and tested, Phase 8 shipped on 2026-08-20, and the API is deployed and serving the current commit. Nothing between here and a running shop is code any more — it is the four operator tasks in [06](./06-development-guide.md#running-in-production): a real certificate, the six mail variables, and the customer and audit sweeps in cron.

*This paragraph and the snapshot above read “as of 2026-08-20” until 2026-09-02, quoting 701 and 165 tests and describing Phase 8 as the thing still ahead — three weeks after it landed. A snapshot with a date on it is a claim like any other, and this one had stopped being true.*
...
| ~~Server-side PDF invoices~~               | ~~FR-BILL-18~~                                      | **Superseded 2026-08-31, not cancelled and not deferred.** It was on this list when the printed bill was a receipt, and "Puppeteer or pdfkit" was the note. FR-BILL-20 now prints a full GST tax invoice that the browser saves as a PDF named after the invoice number, so the document exists without either dependency — Puppeteer in particular would have pulled a Chromium into a backend image whose whole deployment story is one small container. What is still unserved is emailing a bill with no browser open, which nobody has asked for and which needs the same SMTP decision FR-AUTH-11 waits on; if that ever lands, revisit this then and not before |
...
| ~~Audit log for stock and price changes~~ | NFR-17 | **Done 2026-08-22.** Reads deliberately not logged. Shipped as a Prisma middleware and **moved to a client extension 2026-08-31** so the audit row joins the caller's transaction (Phase 13). Its 24-month retention was decided the day it shipped and **enforced from 2026-08-31** by `npm run purge:audit` — nine days in which four documents described a rule nothing applied |
| ~~Password reset by email~~                | FR-AUTH-11                                          | **Done 2026-08-31.** Brought SMTP as the stack's first outbound dependency — which docs/07 P1-6 had declined for the breach check, on reasoning that does not carry across: mail is the *delivery* of something the user asked for, not a control that fails open. Self-service signup is what made it necessary, since a shopkeeper who opened their own shop has no administrator to ask |
