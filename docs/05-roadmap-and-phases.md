...
| Test coverage                           | 576 backend tests (~92% statements) with a 90% gate on the money and auth paths, 115 frontend unit tests, and a 7-flow browser smoke — all three on CI. gates on the invoice, auth, dashboard and trend paths; `medicine.controller.js` at 74.5% is now the largest remaining gap |
