...
| `hsnCode`                   | String?      | optional, **free text** | HSN for GST classification; searchable. The medicine form offers the twelve codes a pharmacy uses with what each covers (`frontend/src/lib/hsn.ts`) and takes any other, because those twelve are nowhere near the whole schedule. **Deliberately not format-validated:** the catalogue already holds 5-, 6- and 8-digit codes and one non-numeric value, and a rule that rejected them would make those rows uneditable — the failure FR-MED-07's enum caused with `unit` |
| `unit`                      | String       | required           | The dispensing unit. Bounded by Zod — 1–20 chars, starts with a letter, lower-cased on write — but **not** enumerated and **not** constrained by the database. It listed nine fixed values until 2026-09-01; see [04 §medicines](./04-api-reference.md). `GET /api/medicines/units` reports the distinct ones in use, which is what the medicine form offers back |
...
| 6      | `Medicine.unit` is a free string in the DB                          | Direct DB writes bypass the Zod **shape** check — length, leading letter, permitted characters, lower-casing. *(This said "the Zod allowlist" until 2026-09-04. There has been no allowlist since 2026-09-01: FR-MED-07 removed the nine-value enum deliberately, because it made a shop selling vials or sachets file them under `other` and print that on the customer's invoice.)* | A Postgres `CHECK` matching the Zod regex — **not** an enum, which would reinstate the closed list the requirement was changed to remove |
