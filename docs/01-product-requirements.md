...
| NFR-23 | Maintainability | Automated test coverage                                          | ✅ **631 backend tests** across 24 files (Vitest + Supertest, measured 2026-08-31), frontend unit tests (Vitest + Testing Library) and a 7-flow Playwright smoke, all on CI, with a 90% gate on the invoice, auth and dashboard paths and 100% on the shared trend query. The frontend count is still due a re-measure on Node 22 — see [09](./09-testing-strategy.md), which is where these numbers live |       |
