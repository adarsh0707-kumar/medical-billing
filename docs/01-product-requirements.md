...
Multi-tenancy (§6.0) · Refresh-token rotation and server-side revocation · Audit log · Invoice void and partial returns · Schedule H prescription register · Customer erasure and retention · Manual stock adjustment · CSV export on every report · Monthly and yearly reports · A GST tax-invoice print layout.

### In scope — planned, not built

Server-side PDF invoices (FR-BILL-18) · Top-selling medicines (FR-RPT-07) · Profit/margin report (FR-RPT-08) · Password reset by email (FR-AUTH-11) · Email/SMS alerts (FR-NOTIF-06).
...
| NFR-14 | Security        | SQL injection resistance                                         | ✅ Prisma parameterises everything. **Six** raw statements exist across five call sites — the two document-serial upserts, the two trend aggregations, the period-report bucketing and the readiness probe — and every one is a bound `$queryRaw` tagged template. `$queryRawUnsafe` appears nowhere. *(This row said "one raw statement" until 2026-08-31; the count grew with the trend and period reports. [SECURITY.md](../SECURITY.md#security-relevant-design) holds the authoritative list — do not start a second count here.)* |       |
...
| NFR-17 | Auditability    | Who-did-what trail for stock and price changes                   | ✅ `AuditLog` since 2026-08-22 — actor, model, record and before/after on every write to master data, recorded by a Prisma middleware so a new write path cannot forget. Reads are deliberately not logged ([03 §3.12](./03-data-model.md#312-auditlog--who-changed-what)); retention decided at 24 months, purge job not yet built |       |
...
| NFR-23 | Maintainability | Automated test coverage                                          | ✅ Backend (Vitest + Supertest), frontend unit (Vitest + Testing Library) and a 7-flow Playwright smoke, all on CI, with a 90% gate on the invoice, auth and dashboard paths and 100% on the shared trend query. **The counts live in [09](./09-testing-strategy.md) and are due a re-measure** — the 604 / 125 figures previously quoted here date from 2026-08-27 and are now understated |       |
