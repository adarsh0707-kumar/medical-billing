...
| FR-RPT-10 | Monthly report: the month's takings, invoices, credit notes and GST, broken down by day | All | ✅ | `GET /api/reports/monthly?month=&year=`, plus `/export`. Zero-filled across the month, so a quiet day is a gap rather than a missing bar |
| FR-RPT-11 | Yearly report: the same figures for a year, broken down by month | All | ✅ | `GET /api/reports/yearly?year=`, plus `/export` |
...
| NFR-23 | Maintainability | Automated test coverage                                          | ✅ 604 backend tests (Vitest + Supertest), 125 frontend unit tests (Vitest + Testing Library) and a 7-flow Playwright smoke, all on CI. ~92% of statements overall, with a 90% gate on the invoice, auth and dashboard paths and 100% on the shared trend query |       |
