...
| NFR-23 | Maintainability | Automated test coverage                                          | ✅ 599 backend tests (Vitest + Supertest), 125 frontend unit tests (Vitest + Testing Library) and a 7-flow Playwright smoke, all on CI. ~92% of statements overall, with a 90% gate on the invoice, auth and dashboard paths and 100% on the shared trend query |       |
