...
Multi-tenancy (§6.0) · Refresh-token rotation and server-side revocation · Self-service password reset over SMTP · Audit log, with its retention enforced · Invoice void and partial returns · Schedule H prescription register · Customer erasure and retention · Manual stock adjustment · CSV export on every report · Monthly, yearly, margin and top-seller reports · A GST tax-invoice print layout.

### In scope — planned, not built

Email/SMS alerts (FR-NOTIF-06). **The dependency it waited on is no longer the obstacle** — FR-AUTH-11 brought SMTP on 2026-08-31, so what remains is the product question of which alerts are worth sending and to whom, not whether the stack can send one.

### In scope — superseded

Server-side PDF invoices (~~FR-BILL-18~~), by the GST tax-invoice print layout (FR-BILL-20) on 2026-08-31. The row is kept in §6.8 with the reasoning, because "we decided not to, and here is why" is more useful to the next reader than a requirement that quietly disappeared.

> **This section was rebuilt on 2026-08-31**, having listed *"Purchase orders & goods receipt (schema exists, no API) · Server-side PDF invoices · Audit log"* — every entry of which had since stopped being true. The purchases schema was **dropped** on 2026-08-24 (Q7), the audit log **shipped** on 2026-08-22, and the PDF invoice was **superseded** later the same day this note was written. Nothing that was on the original list is still planned.
...
| FR-AUTH-11 | Password reset by email                                                                                   | Public | ✅ Since 2026-08-31 | `POST /api/auth/forgot-password` and `POST /api/auth/reset-password`. **Brought SMTP as the stack's first outbound dependency**, which [07 §10 P1-6](./07-security.md#10-hardening-backlog) had declined for the breach check — the difference is that mail is the *delivery* of something the user asked for rather than a control that would fail open, so a failure weakens nothing. Self-service signup is what made it necessary: a shopkeeper who opened their own shop has no administrator to ask. The token is 32 random bytes, **stored only as a SHA-256**, single-use, and dead after 30 minutes; completing a reset bumps `tokenVersion` and revokes every refresh token, the way a password change does. **The request answers identically for a known and an unknown address** — including when the mail server is down, which is why that failure is reported in the log and nowhere else |
...
| FR-MED-12 | Capture a prescription for Schedule H medicines                                                      | All    | ✅ Since 2026-08-24. Any invoice containing a Schedule H line requires a register entry — prescriber, their council registration number, the prescription's date and the patient's name — written in the same transaction as the sale. Schedule H is decided from the **batch's** medicine, never the client-supplied `medicineId`, which is validated but not persisted. See [03 §3.x](./03-data-model.md) and [PRD Q4](#14-open-questions). **Readable since 2026-09-01**: a Prescriptions tab on the Reports page (ADMIN, PHARMACIST) filtered by prescriber, registration number, patient or the date the prescription was *written*, with the CSV an inspection asks for; the particulars also print on the invoice and show on the invoice detail dialog. Between 2026-08-24 and 2026-09-01 the register was recorded and could not be produced — the fourth time this repo has shipped a capability with nothing reaching it (see FR-AUTH-09, FR-BILL-17, FR-RPT-07). |
...
| ~~FR-BILL-18~~ | ~~Server-generated PDF invoice~~                                                                     | All   | **Superseded 2026-08-31 by FR-BILL-20, two rows below. Not built, and not planned.** It was specified when the printed bill was a receipt and a PDF meant leaving the browser to get a proper document. FR-BILL-20 produces a full GST tax invoice, and the browser saves it as a PDF named after the invoice number — so the thing this row was reaching for exists. What a server renderer would still add is **emailing a bill with no browser open**, which nobody has asked for in the four months since 1.0.0 and which needs the SMTP dependency [FR-AUTH-11](#61-authentication--session--fr-auth) is also waiting on. Kept rather than deleted: a requirement that was superseded tells the next reader why not to build it, where a row that vanished tells them nothing |
...
| FR-RPT-07 | Top-selling medicines                                                                                                 | All               | ✅ API since 2026-08-31, **reachable from the UI since 2026-09-01** — a Top Sellers tab on the Reports page with the period, a Top 10/20/50 selector and the CSV export. Between those dates the endpoint existed and no screen called it, which is the third time this repo has shipped that shape (see FR-AUTH-09 and FR-BILL-17) and the reason the row now says both dates. `GET /api/reports/top-sellers?month=&year=&limit=`, plus `/export`. Ranked by units sold, with the value of what sold. **Grouped by `Batch.medicineId`, never by `InvoiceItem.medicineName`** — that column is a sale-time snapshot (BR-12), so grouping by it splits one medicine into two rows after a rename and each half ranks lower than the whole. The name shown is the medicine's current one. Returns come off the units in the month the credit note was issued, leaving the sale's own month intact (BR-14); a medicine returned in full drops off rather than appearing at zero. `limit` is the shared validated one, capped at 100. Open to every role — this is what the shop sold, not what it cost, which is the line between it and FR-RPT-08 |
| FR-RPT-08 | Profit/margin report (selling vs purchase price)                                                                      | ADMIN             | ✅ API since 2026-08-31, **reachable from the UI since 2026-09-01** — a Profit & Margin tab showing revenue, cost of goods, profit and margin %, with a daily bar chart and the CSV export. The tab is filtered out for anyone but an ADMIN, the way the GST tab is, and the page does not request the report for a role the server would refuse. `GET /api/reports/margin?month=&year=`, plus `/export`. A month's revenue against what the stock cost, broken down by day and zero-filled. **Revenue is `subtotal − discountAmt`** — what the shop keeps before tax, not `totalAmount`, which carries GST the shop only collects and would overstate profit by it. **Cost is the batch's `purchasePrice` at the quantity sold**, negated for a credit note, so returned stock takes its cost off the period that took it back. Both come from stored columns; nothing is re-derived ([G-21](./08-gap-analysis.md#g-21)). **ADMIN only** — the deliberate contrast with FR-RPT-10/11, which every role may read: a shop's takings are its own trading record, what its stock cost is not. A batch with a zero recorded cost is counted in `unpricedLines` rather than passing as free stock, because the two are indistinguishable in the arithmetic and one of them reads as a flawless month |
...
| FR-NOTIF-06 | Email or SMS alerts | — | ⬜ **No longer blocked, and a smaller question than it was.** It waited on there being no way to send mail; since FR-AUTH-11 landed on 2026-08-31 there is one, and `config/mailer.js` is a consumer away from serving this too. What stays unbuilt is the product decision — which alerts are worth an email, to whom, and how often — not the dependency. SMS is a separate provider and is not implied by this |
...
| NFR-23 | Maintainability | Automated test coverage                                          | ✅ **701 backend tests** across 28 files (Vitest + Supertest) and **165 frontend unit tests** across 20 files (Vitest + Testing Library), plus a 7-flow Playwright smoke — all three on CI, all measured 2026-08-31. 91.8% of backend statements, with 90% gates on the invoice, auth and dashboard paths and 100% on the shared trend query. [09](./09-testing-strategy.md) is where these numbers live; take them from a run rather than from adding up its tables |       |
