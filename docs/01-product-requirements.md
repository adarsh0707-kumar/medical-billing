...
| FR-AUTH-09 | Server-side logout / token revocation                                                                     | All    | ⬜     | Logout is client-side only — the JWT stays valid until expiry                 |
...
| FR-MED-12 | Block or warn on sale of a Schedule H item without a recorded prescription                                                                      | —                | ⬜ Flag is displayed only; nothing is enforced |
...
| FR-BATCH-07 | Update a batch (price corrections, quantity adjustment)                                              | ADMIN, PHARMACIST | 🟡 No schema validation on this route — see[G-05](./08-gap-analysis.md#g-05)                                                        |
| FR-BATCH-08 | Record manufacture date alongside expiry                                                             | —                | 🟡 Column and controller handling exist; the validator strips the field so it is always`null` — [G-04](./08-gap-analysis.md#g-04) |
| FR-BATCH-09 | Prevent sale from an expired batch                                                                   | —                | ⬜ Expiry is surfaced but never blocks a sale                                                                                       |
| FR-BATCH-10 | Stock is decremented automatically when an invoice is created                                        | —                | ✅                                                                                                                                  |
| FR-BATCH-11 | Manual stock adjustment with a reason/audit trail                                                    | ADMIN             | ⬜                                                                                                                                  |
...
| FR-BILL-11 | Human-readable sequential invoice number,`INVyymmdd-nnnn`                                               | —    | 🟡 Generated from a same-day count — collides under concurrency,[G-01](./08-gap-analysis.md#g-01) |
...
| NFR-02 | Performance     | List endpoints are paginated                                     | ✅ Medicines, customers, invoices. ❌ Batches, suppliers, categories, manufacturers, users return unbounded sets                   |       |
...
| NFR-11 | Security        | CORS restricted to an allowlist                                  | 🟡 Allowlist omits the Nginx origin`http://localhost` — [G-02](./08-gap-analysis.md#g-02)                                        |       |
| NFR-12 | Security        | Rate limiting on the API                                         | 🟡 500 req / 15 min, but`trust proxy` is unset so all traffic behind Nginx shares one bucket — [G-06](./08-gap-analysis.md#g-06) |       |
| NFR-13 | Security        | Input validation on request bodies                               | 🟡 Zod on most POST/PUT; missing on batch update, user create/update, auth register                                                |       |
| NFR-14 | Security        | SQL injection resistance                                         | ✅ Prisma parameterises everything; no raw SQL in the codebase                                                                     |       |
...
| NFR-23 | Maintainability | Automated test coverage                                          | ❌`npm test` exits 1; no test files exist                                                                                        |       |
