...
| D-6                   | `npm test`, `npm run test:coverage`, `npm run test:watch`, `npm run lint`, `npm run format` for the backend | backend + root README          | ~~Only `test` exists and it exits 1~~ — the three test scripts are real as of 2026-08-19. ~~There is still no backend lint or format script~~ — **closed 2026-08-24**: `lint`, `lint:fix`, `format` and `format:check` all exist, and `npm run lint` runs in the backend CI job. `format` is deliberately not a gate — see the note below                                                                                                                                                                                        |
...
- **D-6** — **closed 2026-08-24.** ESLint flat config in `backend/eslint.config.js`, on the frontend's version line, written as CommonJS because that is what this half of the repo is. Seven findings, all fixed rather than suppressed: three boot messages moved from `console.log` to pino (they were the first two lines in any aggregator, arriving unstructured while everything else was JSON), a redundant `eslint-disable-next-line` that someone had added in anticipation, and a dead `startOfMonth` helper in `invoice.utils.js` — invoice numbering is per-day and nothing was per-month.

  `npm run format` exists but is **not** wired into CI and was **not** run across the tree. Thirteen files differ from Prettier at 80 columns, about 141 lines; measured, a wider `printWidth` makes *more* files differ, not fewer, so there is no setting that turns the script into a no-op. Reformatting them is a legitimate change — it is just not this one, and burying it here would have made the lint fixes unreviewable.
