...
| SQL injection | Prisma parameterises everything. **Seven** raw statements exist, and every one is a bound `$queryRaw` tagged template: the readiness probe (`app.js`), the two document-serial upserts (`invoice.utils.js`), and four aggregations in `utils/trend.js` — the 7-day trend, the period-report bucketing, and the margin report's revenue and cost. `$queryRawUnsafe` appears nowhere ([full list](./docs/07-security.md#5-injection--data-access-safety)). *(Recounted 2026-08-31: this said six, a number that had been taken from a grep which also matched the phrase `$queryRaw` inside a comment. Count the invocations, not the mentions.)* |
